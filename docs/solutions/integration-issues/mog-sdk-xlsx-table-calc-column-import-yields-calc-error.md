...
- [docs/solutions/integration-issues/upstream-mog-open-defects-and-which-lane-they-reach.md](upstream-mog-open-defects-and-which-lane-they-reach.md) — the rest of the upstream tracker, indexed by which of this repo's lanes each defect reaches.
- [docs/solutions/architecture-patterns/engine-binding-decides-the-workbook-size-ceiling.md](../architecture-patterns/engine-binding-decides-the-workbook-size-ceiling.md) — upstream #335 is the separately-filed home of the import *cost* this doc split out. It is a wasm32 memory trap, not a scheduling problem, and it confirms the split was right.

## Upstream status

Filed as **[fundamental-research-labs/mog#337](https://github.com/fundamental-research-labs/mog/issues/337)** on 2026-08-04, against 0.10.5, as a wrong-result evaluation bug. As of 2026-08-06 it is **open with no maintainer reply, no labels, no assignee, and no milestone**. The workbook itself could not be attached — it carries customer financial data — so the issue carries the measurements rather than a repro file.

The upstream defect is therefore unchanged and unacknowledged. The value-fidelity gate in `server/value-fidelity.ts` is what is actually protecting files, and it is what will keep protecting them; filing is not a fix and must not be recorded as one.

An earlier version of this section read *"No matching GitHub issues; this repo tracks related work via PRs rather than issues."* That sentence was about **this** repo's tracker and was read as a statement about upstream's, which at the time already held ten open issues. It is corrected here rather than deleted, per this repo's rule about keeping a wrong diagnosis visible.
