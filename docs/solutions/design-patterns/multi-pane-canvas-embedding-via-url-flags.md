...
2. **`?compact=1`** — slims chrome for small panes: one-row header, no footer, and the embed's ribbon and status bar hidden by CSS ([styles.css](../../../src/styles.css), `.app.compact` rules). The formula bar and sheet tabs stay so it still reads as a spreadsheet.
...
- docs/solutions/ui-bugs/file-origin-compare-page-blank-iframes.md — the page must be served by the dev server, not opened as `file://`.
